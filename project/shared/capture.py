...
The collector runs a passive packet tap (scapy) and parses each DNS packet into a
flat :class:`CaptureEvent`, which it publishes onto the transport spine (topic
``capture``). The defender consumes these and turns *windows* of events into
:class:`~shared.schema.FeatureRecord` rows.
...
@dataclass
class CaptureEvent:
    """One parsed DNS packet observed on the wire."""
